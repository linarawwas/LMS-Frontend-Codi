...
const Students = () => {
  ...
  const attendanceURL = "https://lms-backend-codi-production.up.railway.app/api/attendances";
  ...
  const deleteUser = async (student_id) => {
    const token = localStorage.getItem("token");
    axios.delete(`https://lms-backend-codi-production.up.railway.app/api/attendances/${student_id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
